...
TEST_F(socketCANTest, InvalidSendFrameFD) {
    ...
    // Test 3: NULL data with non-zero length
    {
        ...
        EXPECT_EQ(result, 0); // Also passes - writes zeros
        
        close(s);
    }

    // Test 4: Very large CAN ID (no validation in FD function)
    {
        int s = socketCan_init(validInterface);
        ASSERT_GE(s, 0);
        
        int8_t data[8] = {1, 2, 3, 4, 5, 6, 7, 8};
        // FD function doesn't validate CAN ID, so this might pass
        int result = can_send_frame_fd(s, 0xFFFFFFFF, data, 8);
        // Depending on kernel, might succeed or fail
        // EXPECT_LT(result, 0); // Uncomment if you add validation
        
        close(s);
    }
}

// can_close - Invalid tests
TEST_F(socketCANTest, InvalidClose) {

    // Test 1: Close invalid socket (should not crash)
    {
        can_close(-1); // Should do nothing, not crash
        SUCCEED();
    }
    ...
    // Test 2: Double close
    {
        ...
        can_close(s);
        can_close(s); // Double close - shouldn't crash
        SUCCEED();
    }
    ...
    {
        int s = -1;
        can_close(s); // Should handle gracefully
        SUCCEED();
    }
}